...
function referral_form_enqueue_scripts()
{
    ...
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'referral_form')) {
        $referral_script_url = (defined('WP_DEBUG') && WP_DEBUG) ? 'http://localhost:3000/src/main.js' : plugin_dir_url(__FILE__) . 'dist/assets/main-mER-KV-g.js';
...
function referral_form_enqueue_styles()
{
    global $post;
    if (is_a($post, 'WP_Post') && has_shortcode($post->post_content, 'referral_form')) {
        $referral_style_url = plugin_dir_url(__FILE__) . 'dist/assets/main-9GaZX9X6.css';
